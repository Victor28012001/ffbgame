  ...
  const getAllCharacters = async () => {
    ...
    if (Status) {
      ...
      if (!isAnimating && creatorWarriors.length > 0) {
        ...
        console.log(
          'Initializing dataaaaaaa:.......... ' + charIdMapping.get('opponent_warrior2')
        )
        ...
        for (let i = 0; i < opponentPlaceholder.length; i++) {
          ...
          if (
            opponentPlaceholder[i].id == charIdMapping.get('opponent_warrior1') ||
            opponentPlaceholder[i].id == charIdMapping.get('opponent_warrior2') ||
    ...
    if (
      ...
      if (JSON.parse(duelData.battle_log).length < 1) {
        ...
        interface MessageData {
          ...
          duel_id: number // Duel identifier
          data: {
            // Expected structure of 'data'
            key1: string
            key2: number // Example types
            // Add more keys as needed
          }
        }

        // Constructing dataObject1 with all required fields
        const dataObject1: MessageData = {
          func: 'startDuel', // Example function name
          duel_id: 123, // Example duel ID
          data: {
            key1: 'exampleValue',
            key2: 42, // Example value for key2
          },
        }

        // Calling the function with the corrected object
    ...
    <section className='flex overflow-hidden gap-10'>
      <main className=' w-3/12  mt-12 ml-10'>
        ...
        <p className=' text-center'>
          {' '}
          {duelData
            ? `${duelData.duel_creator.slice(0, 9)}........${duelData.duel_creator.slice(
                -10
              )}`
            : '0x00'}
        </p>
          ...
          {creatorCharacterDetails?.map((item, index) => (
            <div
              className={`w-[80%] grid grid-cols-2 gap-2 border bg-myBlack p-4 h-40 rounded-md ${
                item.health === 0 ? 'border-red-700 border-spacing-5' : 'border-green-400'
              }`}
              key={index}
            >
              <ImageWrap
              ...
              <div className='flex flex-col items-center justify-center'>
                ...
                <p className='text-sm text-gray-400 mb-1'>HLT: {item?.health}</p>
                <p className='text-sm text-gray-400 mb-1'>STR: {item.strength}</p>
