...
import { AdditiveBlending, DoubleSide, MathUtils, MeshBasicMaterial } from "three";
...
export const Speed = () => {
  const speedMaterial = useRef<MeshBasicMaterial | null>(null);
  ...
  useFrame((_state, delta) => {
    if (scroll.offset - lastScroll.current > 0.0005) {
      if (speedMaterial.current) {
        speedMaterial.current.opacity = MAX_OPACITY; // Check if speedMaterial is defined
      }
    }
    lastScroll.current = scroll.offset;

    if (speedMaterial.current && speedMaterial.current.opacity > 0) { // Ensure speedMaterial is defined
      speedMaterial.current.opacity -= delta * 0.2; // Adjust opacity over time
    }
  });
  ...
  return (
    <group>
      <Instances>
        ...
        />
        {Array(INSTANCES)
          .fill(null) // Use null or another value to fill the array
          .map((_, key) => (
            <SpeedShape key={key} />
          ))}
      </Instances>
