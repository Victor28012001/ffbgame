...
const Header = () => {
    ...
    return (
        <header className={`w-full h-auto ${isNavVisible ? "absolute top-0 left-0 z-[80]" : "absolute top-0 left-0 z-[80]"}`}>
            <div className="w-full relative flex justify-between items-center py-8 md:px-8 px-3">
                <Link to='/Home'>
                    <ImageWrap className="md:w-[60px] w-[40px]" objectStatus="object-cover" alt="logo" image={Logo} />
                </Link>

                <ul className="lg:flex lg:justify-between hidden items-center gap-8">
                ...
                <aside className="flex items-center lg:gap-6 gap-2">
                    <Button className="text-gray-100 lg:flex hidden text-xl font-bold hover:text-myGreen" onClick={handleSearchClick} type="button" >
                        <FiSearch />
                    </Button>

                    <WalletButton />

                    <Button className="lg:hidden flex text-3xl border-2 p-1 border-myGreen font-bold text-myGreen" type="button" onClick={handleToggle}>
                        <IoMdMenu />
                    </Button>
                </aside>
                ...
                <div className={`fixed top-0 z-[99] w-full h-screen bg-bodyBg/50 transition-all duration-[500ms] ease-[cubic-bezier(0.86,0,0.07,1)] lg:hidden flex justify-end ${openMenu ? "left-0" : "left-[100%]"}`}>
                    <div className={`w-[80%] h-full bg-bodyBg border-l-2 border-myGreen/10 flex flex-col gap-10 transition-all duration-[500ms] ease-[cubic-bezier(0.86,0,0.07,1)] px-6 py-8 delay-300 ${openMenu ? "translate-x-0" : "translate-x-full"}`}>
                        <header className="flex justify-between items-center w-full">
                            ...
                            <Button type="button" className="text-2xl text-myGreen" onClick={handleToggle}>
                                <GiCrossedSwords />
                            </Button>
                        </header>
...
export const Search = ({ openSearch, handleSearch }: SearchTypes) => {
    ...
    return (
        <div
            ...
            <div className="absolute text-4xl text-myGreen cursor-pointer right-[5%] top-[5%]" onClick={handleSearch}>
                <GiCrossedSwords />
            </div>
                ...
                <div className="flex flex-wrap w-full">
                    <div className="w-full flex flex-col justify-center items-center basis-full relative px-[15px]">
                        ...
                        <form className="relative w-full z-50">
                            ...
                            <Button type="submit" className="absolute text-[30px] text-myGreen border-0 right-5 top-[30%] bg-transparent">
                                <FiSearch />
                            </Button>
                        </form>
                    </div>
