...
const juksbucksMintAddress = new PublicKey(
  "4NDWJ9h84K3osUxfnZ4gAA5t3Z1gDCxtpHQZ4Wj5LpCh"
); // Replace with your Juksbucks mint address

exports.createPlayer = async (req, res) => {
  const { monika, walletAddress, avatarUrl } = req.body;
  ...
  try {
    ...
    // const balance = await connection.getBalance(playerAccount.publicKey);
    // console.log("Balance:", balance);

    // Airdrop SOL to the new account (only for testing; replace with actual funding mechanism)
...
exports.modifyMonika = async (req, res) => {
  const { walletAddress, newMonika } = req.body;
  const success = await playerService.modifyMonika(walletAddress, newMonika);
  if (success) {
    res.status(200).json({ message: "Monika updated successfully" });
  } else {
    res.status(404).json({ error: "Player not found" });
  }
};
